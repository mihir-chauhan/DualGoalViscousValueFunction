...
class GCIVLAgent(flax.struct.PyTreeNode):
    ...
    def eikonal_loss(self, batch, grad_params, key):
        ...
        g1, g2 = self.distance_grad_s(batch['observations'].astype(jnp.float32), batch['value_goals'], grad_params)
    ...
    def value_loss(self, batch, grad_params):
        ...
        if self.config.get('enable_eikonal_reg', True):
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            agent_name='gcivl_eikonal',  # Agent name.
