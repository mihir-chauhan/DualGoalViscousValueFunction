...
class GCIVLAgent(flax.struct.PyTreeNode):
    ...
    def stochastic_fk_loss(self, batch, grad_params, key):
        ...
        v_out, input_embeds = self.network.select('value')(obs, batch['actor_goals'], params=grad_params, return_embedding=return_embedding)
        if len(obs.shape) > 2:
            B, D = input_embeds.shape
            obs = input_embeds
        else:
            B, D = obs.shape
        G = goal_reps.shape[-1]
        ...
        if return_embedding:
            v_neighbors_out = self.network.select('value')(flat_noisy_obs, is_embedding=True, params=grad_params)
        else:
        ...
        if local_speed.ndim==1:
            local_speed = local_speed[:, None]
