...
class GCIVLVisualDualAgent(flax.struct.PyTreeNode):
    ...
    def compute_latent_grad_norm(self, network_key, z, g, params):
        """
        Compute ||grad_z V(z, g)||. 
        Enforces Eikonal constraint on the Latent Manifold.
        """
        def value_sum(embedding, goal, p):
            if network_key == 'rep_value':
                # Rep Value takes (z, z_goal)
                v = self.network.select(network_key)(embedding, goal, params=p)
                return (-v).mean()
            else:
                # Value takes (z, rep_goal)
                # We typically use v1 for the gradient direction
                v1, _ = self.network.select(network_key)(embedding, goal, params=p)
                return (-v1).mean()

        # Compute gradient w.r.t the first argument (embedding)
        # z has shape (batch, latent_dim)
        grad_fn = jax.vmap(jax.grad(lambda _z, _g, _p: value_sum(_z[None], _g[None], _p).squeeze()), in_axes=(0, 0, None))
        
        # Calculate gradients on the latent vector
        grads = grad_fn(z, g, params)
    ...
    def rep_loss(self, batch, grad_params):
        ...
        # Rep value loss.
        q1, q2 = self.network.select('target_rep_critic')(obs, goals, batch['actions'][..., None])
        q = jnp.minimum(q1, q2)
        ...
        q1, q2 = self.network.select('rep_critic')(grad_obs, grad_goals, batch['actions'][...,None], params=grad_params)
    ...
    def viscous_dual_loss(self, batch, grad_params, key):
        """
        ...
        obs_embeddings = self.network.select('state_encoder')(batch['observations'], params=grad_params)
        goal_embeddings = self.network.select('goal_encoder')(batch['value_goals'], params=grad_params)
        if local_speed.ndim == 1: local_speed = local_speed[:, None]
        goal_reps = self.network.select('rep_value')(goal_embeddings)
        B, D = obs_embeddings.shape
        G = goal_reps.shape[-1]

        # 3. Get V(s) [Anchor]
        psi_out = self.network.select('value')(obs_embeddings, goal_reps, params=grad_params)
        
        
        # Ensemble handling
        if isinstance(psi_out, tuple):
            psi_s = (psi_out[0] + psi_out[1]) / 2.0
        elif hasattr(psi_out, 'shape') and psi_out.shape[0] == 2 and psi_out.ndim > 1:
            psi_s = jnp.mean(psi_out, axis=0)
        else:
            psi_s = psi_out
            
        if psi_s.ndim == 1: psi_s = psi_s[:, None] # (B, 1)

        # 4. Generate Neighbors (s + epsilon)
        obs_expanded = jnp.repeat(obs_embeddings[:, None, :], K, axis=1)
        ...
        psi_neighbors_out = self.network.select('value')(flat_noisy_obs, flat_goal, params=grad_params)
        
        if isinstance(psi_neighbors_out, tuple):
            psi_neighbors_flat = (psi_neighbors_out[0] + psi_neighbors_out[1]) / 2.0
        elif hasattr(psi_neighbors_out, 'shape') and psi_neighbors_out.shape[0] == 2 and psi_neighbors_out.ndim > 1:
            psi_neighbors_flat = jnp.mean(psi_neighbors_out, axis=0)
        else:
            psi_neighbors_flat = psi_neighbors_out
            
        psi_neighbors = psi_neighbors_flat.reshape(B, K) # (B, K)
        ...
        abs_diff = jnp.abs(psi_neighbors - psi_s)#/jnp.linalg.norm(psi_s + 1e-6)
        const_psi = jax.lax.stop_gradient(psi_s)
        
        # Estimate the denominator using Triangle Inequality \Psi
    ...
    def value_loss(self, batch, grad_params):
        """Viscous IQL Value Loss (Visual/Latent)."""
        # 1. Encode
        grad_obs, grad_next_obs = (
            self.network.select('state_encoder')(batch['observations'], params=grad_params),
            self.network.select('state_encoder')(batch['next_observations'], params=grad_params),
        ...
        # 2. Get Goal Reps (For the Driver)
        # Note: In visual mode, goal_reps usually come from rep_value(goals)
        goal_reps = self.network.select('rep_value')(goals)
        
        # 3. Standard IQL Targets
        ...
        # 4. Current Value (With Gradients on Encoder + Value Head)
        # We need goal_reps with gradients if we want to propagate back to goal_encoder?
        # Usually HIQL stops gradient on goal rep for the low-level, but here for Value it's okay.
        ...
        if self.config.get('enable_fk_regularization', True):
            v_loss, v_loss_dict = self.viscous_dual_loss(batch, grad_params, self.rng)
            total_loss += v_loss
    ...
    def create(
        ...
        network_info = dict(
            ...
            rep_critic=(rep_critic_def, (ex_encoder_outputs, ex_encoder_outputs, ex_actions[...,None])),
            target_rep_critic=(copy.deepcopy(rep_critic_def), (ex_encoder_outputs, ex_encoder_outputs, ex_actions[...,None])),
